...
interface FormTemplateProps {
  ...
  pageTitle: DictionaryLeaves
  step: number
  agencyName: string
}

const FormTemplate: FC<FormTemplateProps> = ({ children, title, description, pageTitle, step, agencyName }: FormTemplateProps) => {
  const { t } = useTranslation()

  return (
    <>
      <Head>
        <title>{t(pageTitle)} | {agencyName}</title>
      </Head>
      <Box
        marginTop={'1.5rem'}
        display={'flex'}
